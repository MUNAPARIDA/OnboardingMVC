...
namespace OnBoardingGIC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    {
                        if (db.CandidateDetails.Any(m=>m.UserId == user.Id))
                        {
        ...
        public async Task<ActionResult> Register([Bind(Exclude ="UserPhoto")]RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await roleManager.CreateAsync(new IdentityRole("HRConsultant"));
                    await UserManager.AddToRoleAsync(user.Id, "HRConsultant");*/

                    /*var roleStore = new RoleStore<IdentityRole>(new ApplicationDbContext());
                    var roleManager = new RoleManager<IdentityRole>(roleStore);
                    await roleManager.CreateAsync(new IdentityRole("HRManager"));
                    await UserManager.AddToRoleAsync(user.Id, "HRManager");*/
               ...
               /*     
                    ...
                    mail.From = new MailAddress("reshmaparida2016@gmail.com");
                    mail.Subject = "Credentials";
                    string Body = "Following are your login credentials " + "Username : "+model.Email + "  Password : " + model.Password;
        ...
        public ActionResult OfferSend(string id)
        {
            var user = db.Users.Find(id);
            user.Accepted = true;
